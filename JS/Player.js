...
Player.prototype.createPlayer = 	
	function(Box){
		...
		document.onkeydown = this.setMoveHandler.bind(this); //funzione che consente il movimento dei giocatori;
		document.onkeyup = this.resetMoveHandler.bind(this);
		var loop = setInterval(this.playerMoveHandler.bind(this),20);
	}	

Player.prototype.setImageHome =
	function(){
		//Germania
...
Player.prototype.setImageGuest =
	function(){
		//Germania
		this.Germania = document.createElement("div");
		this.Germania.setAttribute("id","Nazionali");
...
Player.prototype.SceltaTrasferta =
	function(){
		if (NomeCasa != NomeTrasferta && Scelta === 0){
			GiocaTrasferta = 1;
...
Player.prototype.setStartingPosition = 
	function(){
		this.HomeX = 50; //100
 		this.HomeY = 312; //275
 		//posizione iniziale giocatore Guest;
...
Player.prototype.playerMoveHandler = 
	function(evt){
		if (play === true){
...
Player.prototype.setMoveHandler = 
	function(evt){
...
Player.prototype.resetMoveHandler = 
	function(evt){
 		if (evt.which == this.UP)
			this.PressUP = false;
		if (evt.which == this.DOWN)
