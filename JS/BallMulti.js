...
Ball.prototype.Goal = //funzione che gestisce i Goal;
  function(Campo,last){
    if (this.posY >= 175 && this.posY <= 485){ //controllo che la palla sia entrata in rete; 
        ...
        lastGoal = last;
        play = false;
        Campo.player.setStartingPosition(); //reset posizioni iniziali dei giocatori;    
...
Ball.prototype.PlayerHit = 
  function(Campo){
    var collisione = (((Campo.player.HomeX + 10 - this.posX) * (Campo.player.HomeX + 10 - this.posX)
                     + (Campo.player.HomeY + 10 - this.posY) * (Campo.player.HomeY + 10 - this.posY)) <= 8100)
          if (collisione){
              if (this.posX > Campo.player.HomeX + 10)
                this.angle = Math.acos((Campo.player.HomeY + 10 - this.posY) / 90);
              else 
                this.angle = -Math.acos((Campo.player.HomeY + 10 - this.posY) / 90);
              this.setSpeed();
          }
    var impatto = (((Campo.player.GuestX + 10 - this.posX) * (Campo.player.GuestX + 10 - this.posX)
                  + (Campo.player.GuestY + 10 - this.posY) * (Campo.player.GuestY + 10 - this.posY)) <= 8100)
        if (impatto){
            if (this.posX > Campo.player.GuestX + 10)
              this.angle = Math.acos((Campo.player.GuestY + 10 - this.posY) / 90);
            else 
              this.angle = -Math.acos((Campo.player.GuestY + 10 - this.posY) / 90);
            this.setSpeed();
        }
